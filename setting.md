...
## 3. 아키텍처 및 개발 참고 사항 (Architecture & Development Notes)

최근 시스템의 안정성과 확장성을 위해 다음과 같은 구조적 개선이 이루어졌습니다.

### 3.1 통신 아키텍처 (SSE vs WebSocket)
- **리소스 모니터링**: 백엔드의 시스템 지표(CPU, RAM 등)를 클라이언트로 푸시할 때는 **SSE(Server-Sent Events)**를 사용합니다.
- **원격 터미널**: 양방향 실시간 데이터 전송이 필요한 터미널 I/O에는 **WebSocket**을 사용합니다.
- **분리 사유**: 터미널에서 발생하는 대량의 스트리밍 데이터가 모니터링 이벤트의 처리를 지연시키는 **Head-of-line blocking**을 방지하고, 각 채널의 독립적인 에러 핸들링을 보장하기 위한 명시적인 아키텍처 결정입니다.

### 3.2 터미널 동적 리사이징 (JSON Multiplexing)
WebSocket을 통한 터미널 통신은 원시 바이너리(Raw binary)가 아닌 **구조화된 JSON**을 사용합니다.
- `{"type": "input", "data": "..."}` : 사용자 입력 전달
- `{"type": "resize", "cols": 80, "rows": 24}` : PTY 사이즈 변경
이를 통해 PTY 세션을 끊지 않고도 클라이언트 창 크기에 맞춘 실시간 동적 터미널 리사이징을 완벽하게 지원합니다.

### 3.3 코드베이스 및 프론트엔드 안정성 (Codebase Cleanup & Stability)
- `backend/internal` 및 `fyne-frontend/internal`의 방대한 스파게티 로직과 미사용 레거시 코드(`/hello` 등)를 모듈 단위로 분리하고 정리했습니다.
- Fyne 프론트엔드는 Deprecated된 `container.NewMax` 대신 `container.NewStack`을 사용하며, 파일 및 URL 경로 조합에는 항상 `path.Join` 또는 `filepath.Join`을 사용하여 크로스 플랫폼 안정성을 확보했습니다.
