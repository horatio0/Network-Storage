...
- **구현 후 `scripts/agent-check.sh fast` 수동 실행 및 통과**

사용 예시:

```text
/HARNESS-STRICT
WebRTC 화면 렌더링 위젯을 수정해줘
```

```text
./HARNESS-STRICT
새 Tailscale 상태 표시 아이콘을 추가해줘
```
...
5. 구현을 진행한다.
6. **`scripts/agent-check.sh fast`로 기본 검증을 수행한다.**
7. 필요하면 `scripts/agent-check.sh full`로 UI 또는 네트워크 의존 테스트까지 확인한다.
...
- 승인된(수정된) Go 파일의 함수/메서드 길이 제한을 검사한다.
- `gofmt -l` 기반 포맷 검사를 수행한다.
- 소켓 의존 패키지를 제외한 기본 `go test`를 수행한다.
- 승인된(수정된) 변경을 기준으로 문서 동기화가 필요한지 검사한다.
- Fyne 에셋 번들링 상태를 확인한다.

### full

- fast 단계의 모든 검사를 포함한다.
- `go test ./...`를 전체 실행한다.
- X11이나 Wayland 같은 디스플레이 서버 환경이 없으면 UI 테스트가 실패할 수 있다.

## 문서 동기화

다음 변경은 기본적으로 관련 문서 갱신이 필요하다.

- 새로운 UI 화면 또는 클라이언트 기능 추가
- 패키지 책임이나 디렉토리 구조 변경
- 타입 의미나 API 계약 변경

**API 문서 위치:** 
- 백엔드 서버와의 네트워크 연동(모니터링, 파일 전송, 터미널, WebRTC 시그널링 등) 명세 및 가이드는 `docs/api/` 디렉터리에 있다. 클라이언트 연동 작업 시 이 디렉터리 내부 문서들을 반드시 참고해야 한다.
