...
프로젝트의 전체적인 계획과 구현 순서는 아래 문서에 있다.
- `docs/plan/plan.md` 프로젝트의 전체 계획이다.
- `docs/plan/specific.md` 백엔드 구현의 순서이다.

## 공식 명령

### `/HARNESS-STRICT` 또는 `./HARNESS-STRICT`

이 저장소의 공식 하네스 강제 명령이다. 앞의 `./` 유무는 같은 의미로 취급한다.
...
4. 구현할 파일, 함수, 메서드, 타입을 먼저 설명하고 `Approved-Files`에 반영한 뒤 사용자 승인 전까지 코드 작성하지 않는다.
5. 구현을 완료한다.
6. **`scripts/agent-check.sh fast`를 실행하여 최종 검증한다.**

예:

```text
/HARNESS-STRICT
데스크탑 전원을 켜기 위한 WoL 기능을 추가해줘
```
...
5. 구현을 진행한다.
6. **`scripts/agent-check.sh fast` (필요시 `full`)를 실행하여 모든 가드레일을 통과하는지 확인한다.**
