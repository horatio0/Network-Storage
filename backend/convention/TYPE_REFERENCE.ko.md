...
이 패키지는 중앙 통제 컴퓨터의 시스템 리소스 상태를 수집하고 반환하기 위한 도메인 타입이다. 클라이언트(Fyne) 측과 JSON 구조를 동기화하기 위해 존재한다.

### `SystemStatus`

역할:
- 서버의 실시간 리소스(CPU, 메모리, 온도 등) 상태를 표현한다.
- 클라이언트 대시보드에서 JSON 파싱 기준으로 사용된다.

대표 필드 (JSON 키):
- `cpuPercent`: 전체 CPU 사용률 (%)
- `memTotal`: 전체 물리 메모리 (bytes)
- `memUsed`: 사용 중인 메모리 (bytes)
- `memPercent`: 메모리 사용률 (%)
- `temp`: 시스템 최고 온도 (Celsius, 주로 CPU 온도)

왜 필요한가:
- 서버와 클라이언트 간의 상태 모니터링 API (`/api/v1/monitor`)의 계약(Contract) 역할을 한다.
